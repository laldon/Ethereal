...
const int ThreatRookAttackedByMajor  = S( -16,  -5);
...
int evaluateThreats(EvalInfo *ei, Board *board, int colour) {
    ...
    // Penalty for major threats against our rooks
    count = popcount(rooks & attacksByMajors);
    eval += count * ThreatRookAttackedByMajor;
    if (TRACE) T.ThreatRookAttackedByMajor[US] += count;
